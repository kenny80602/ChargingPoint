...
import * as ChangeAvailabilityConst from '@pretso/ocpp/dist/commands/ChangeAvailability.js'
import * as RemoteStartTransactionConst from '@pretso/ocpp/dist/commands/RemoteStartTransaction.js'
import * as StartTransactionConst from '@pretso/ocpp/dist/commands/StartTransaction.js'
...
export default class PretsoChargePoint {
    constructor() {
        let connector1 = new Connector(1)
        let connector2 = new Connector(2)
        this.instance = new ChargePoint({
            // centralSystemUrl: `ws://centralsystem.azurewebsites.net/GavinLocalTest`,
            // centralSystemUrl: `ws://ocpp-centralsystem.pretso.green/GavinLocalTest`,
            centralSystemUrl: `ws://20.184.2.244:9220/Pretso_CP_Simulator`,
            // centralSystemUrl: `http://localhost:9220/Pretso_CP_Simulator`,
    ...
    async run() {
            ...
            try {
                switch (true) {
                    case command instanceof OCPPCommands.RemoteStartTransaction:
                        this.startTransaction(command, true)
                        return {
                    ...
                    case command instanceof OCPPCommands.ChangeAvailability:
                        let changeAvailability_conf = await this.changeAvailability(command)
                        return changeAvailability_conf
                }
            } catch (error) {
                console.log(error)
            }
        ...
        const boot = new OCPPCommands.BootNotification({
            chargePointVendor: 'ECS-ITS-SA500',

            chargeBoxSerialNumber: 'SR' + Math.round(Math.random() * 100000),
            chargePointSerialNumber: 'Pretso_CP_Simulator',
            chargePointModel: 'LIVA-No.1',
        })
        ...
        //     idTag,
        // })
        // console.log('Auth.req :', authCommand)

        // let auth_log = await this.instance.send(authCommand)
        // console.log('auth:', auth_log)
    }

    async startTransaction({ connectorId, idTag }, isRemote) {
        setImmediate(async () => {
            if (!isRemote) {
                const authCommand = new OCPPCommands.Authorize({
                    idTag,
            ...
            if (reservationId) {
                StartTransactionCMD.transactionId = reservationId
            }
            ...
            console.log('TransactionId', start_transaction.transactionId)
            if (start_transaction.idTagInfo.status === StartTransactionConst.STATUS_ACCEPTED) {
    ...
    async stopTransaction({ transactionId }) {
        ...
        console.log('before stop ', connector)
        const statusCommand = new OCPPCommands.StatusNotification({
        ...
        console.log('stop_transaction_Command:', stopCommand)
        clearInterval(this.meterGen)
        let stop_transaction = await this.instance.send(stopCommand)
        //刪除transaction物件，用於其他情境判斷是否正在交易中()
        delete connector.transaction

        console.log('stop_transaction_result:', stop_transaction)
    }
    ...
    async changeAvailability(command) {
        let connector = this.instance.options.connectors.find((element) => element.connectorId === command.connectorId)
        if (command.type === ChangeAvailabilityConst.TYPE_INOPERATIVE) {
            if (connector.status === StatusNotificationConst.STATUS_AVAILABLE) {
                setTimeout(() => {
                    connector.status = StatusNotificationConst.STATUS_UNAVAILABLE
                    this.toggleStatus(command.connectorId, StatusNotificationConst.STATUS_UNAVAILABLE)
                }, 0)
                return { status: ChangeAvailabilityConst.STATUS_ACCEPTED }
            } else return { status: ChangeAvailabilityConst.STATUS_REJECTED }
        } else if (command.type === ChangeAvailabilityConst.TYPE_OPERATIVE) {
            if (connector.status === StatusNotificationConst.STATUS_UNAVAILABLE) {
                setTimeout(() => {
                    this.toggleStatus(command.connectorId, StatusNotificationConst.STATUS_AVAILABLE)
                }, 0)
                return { status: ChangeAvailabilityConst.STATUS_ACCEPTED }
            } else return { status: ChangeAvailabilityConst.STATUS_REJECTED }
        }
